    ...
    try {
        ...
        if (!resumeText) {
            return new NextResponse(
                JSON.stringify({ message: 'Resume text is required' }),
                { status: 400 }
            );
        }
        ...
        const linkedinUrl =
            'https://www.linkedin.com/in/lukk3vdebarezz99l8yy/';
        const portfolioUrl =
            'https://dennis--dennisportfolio-35723.us-central1.hosted.app/';
        ...
        for (let line of lines) {
            ...
            if (
                isBold &&
                sectionTitles.includes(lineText.toUpperCase())
            ) {
                y -= sectionSpacing;
            }
            ...
            const effectiveMaxWidth =
                maxWidth - (isBullet ? bulletIndent : 0);
